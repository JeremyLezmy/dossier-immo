...
export function OverviewStep({ dossier, derived, issues, onSelectIssue }: { readonly dossier: Dossier; readonly derived?: DerivedDossier | undefined; readonly issues: readonly ValidationIssue[]; readonly onSelectIssue: (path: string) => void }) {
  const validatableSteps = editorSteps.filter((step) => step.paths.length > 0);
  const completed = validatableSteps.filter((step) => step.paths.every((prefix) => !issues.some((issue) => issue.path === prefix || issue.path.startsWith(`${prefix}.`)))).length;
  ...
  return <><SectionIntro title="Votre dossier immobilier" description="Toutes les informations restent sur cet appareil. Le fichier téléchargé est la sauvegarde officielle ; le navigateur ne conserve qu'un brouillon de confort." />
    ...
    {issues.length > 0 && <section className="validation-summary" aria-labelledby="validation-title"><h3 id="validation-title">Points à corriger</h3><p>Sélectionnez un point pour ouvrir directement l'étape et le champ concernés.</p><ol>{issues.slice(0, 30).map((issue) => <li key={`${issue.path}-${issue.message}`}><button type="button" onClick={() => onSelectIssue(issue.path)}><span><code>{issue.path}</code><span>{issue.message}</span></span><strong>Corriger</strong></button></li>)}</ol></section>}
  </>;
}
