...
export function HouseholdStep({ form }: { readonly form: UseFormReturn<Dossier> }) {
  ...
  return <><SectionIntro title="Foyer" description="Décrivez la situation actuelle et les évolutions déjà envisagées. Les projections restent explicitement distinctes des faits observés." help="Les identifiants techniques sont générés automatiquement ; vous ne renseignez que les informations utiles au dossier bancaire." />
    ...
    <h3>Personnes du foyer</h3><div className="stack">{people.fields.map((person, index) => <ArrayCard key={person.id} title={form.watch(`household.people.${index}.displayName`) || `Personne ${index + 1}`} subtitle={index === 0 ? "Emprunteur principal" : "Co-emprunteur ou personne à charge"} onRemove={people.fields.length > 1 ? () => people.remove(index) : undefined}>
    ...
    <h3>Évolutions du foyer envisagées</h3><p className="section-note">Ces éléments sont des hypothèses de prudence, jamais présentés comme une situation actuelle.</p><div className="stack">{events.fields.map((event, index) => <ArrayCard key={event.id} title={form.watch(`household.plannedHouseholdEvents.${index}.label`) || `Événement ${index + 1}`} onRemove={() => events.remove(index)}>
