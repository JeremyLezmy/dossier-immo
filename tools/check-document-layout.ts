...
if (sources.length === 0) {
  process.stderr.write("Usage : vite-node tools/check-document-layout.ts <dossier.json> [...]\n");
  process.exit(2);
}
...
try {
  for (const sourceArgument of sources) {
    ...
    await page.setContent(renderBankDocument(dossier, calculateDossier(dossier)), { waitUntil: "load" });
    const report = await page.locator("section.page").evaluateAll((pages) => pages.map((element, index) => ({
      page: index + 1,
      horizontalOverflow: element.scrollWidth - element.clientWidth,
      verticalOverflow: element.scrollHeight - element.clientHeight,
    })).filter((item) => item.horizontalOverflow > 2 || item.verticalOverflow > 2));
    await page.close();
    if (report.length > 0) throw new Error(`${source} déborde : ${JSON.stringify(report)}`);
    process.stdout.write(`${source} : 13 pages sans débordement.\n`);
  }
} finally {
  await browser.close();
}
