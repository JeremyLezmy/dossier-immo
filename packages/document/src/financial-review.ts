...
export function renderFinancialReview(
  ...
  ${
    reading.automatic
      ? `<table class="compact"><thead><tr><th>Date et revenus pris en compte</th><th class="num">${escapeHtml(reading.primaryLabel)}</th><th class="num">Effort¹</th><th class="num">${escapeHtml(reading.prudentLabel)}</th><th class="num">Effort¹</th></tr></thead><tbody>
  ...
  </tbody></table><p class="small">¹ Charges annuelles maximales du scénario central, assurance et crédits simultanés inclus. ${reading.endingIncomes.length ? reading.endingIncomes.map((income) => `${escapeHtml(income.label)} exclu à l’achat, fin le ${formatDate(income.endDate)}.`).join(" ") : "Revenus actifs à la date d’achat."} ${reading.hasBankForecast ? "Les bases incluent des CA prévisionnels, à actualiser avec le réalisé." : "Exercices de CA déclarés réalisés."} La trajectoire économique du budget de vie reste distincte.</p>`
  ...
  <table class="compact"><thead><tr><th>Option</th><th class="num">Somme à régler</th><th class="num">Réserve conservée</th><th class="num">Effort à l’achat</th></tr></thead><tbody><tr><td>Maintien des crédits en cours</td><td class="num">—</td><td class="num">${formatEuro(scenario.reserveForObjectiveCents)}</td><td class="num">${formatRate(scenario.effortRateCentralBasisPoints)}</td></tr>${derived.prepaymentComparisons.map((item) => `<tr><td>Clôture : ${escapeHtml(item.label)}</td><td class="num">${formatEuro(item.settlementCents)}</td><td class="num">${formatEuro(item.reserveForObjectiveCents)}</td><td class="num">${formatRate(item.effortBasisPoints)}</td></tr>`).join("")}</tbody></table>
  <p class="small">Comparaison à apport immobilier inchangé, remboursement prélevé sur la réserve (installation ${dossier.reservePolicy.includesInstallation ? "incluse" : "déduite"}, hors impôts réservés). Décompte du prêteur et maintien des services ou garanties à confirmer. Les ratios retiennent le maximum annuel des charges, assurance comprise ; un crédit court ne disparaît pas du calcul initial.</p>
