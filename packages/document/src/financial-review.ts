...
export function renderFinancialReview(
  ...
  <tr><td>Convention historique avec contrats actifs aujourd’hui</td><td class="num">${formatEuro(reading.bankNowCents)}</td><td class="num">${formatRate(derived.bankReadingEffortBasisPoints.now)}</td><td>Étude préparatoire ; contrats et renouvellements documentés.</td></tr>
  ...
  <p class="small">${dossier.liabilities
    ...
    .join(" ")}</p>
  <p class="small"><strong>Épargne après achat à revenus constants :</strong> ${(
    derived.budgetSavingsPhases[central.id] ?? []
  )
    .slice(0, 3)
    .map(
      (item) =>
        `${formatDate(item.date)} : ${formatEuro(item.savingsCents)} / mois`,
    )
...
export function renderRateComparison(
  ...
  return `<h3>Conditions de financement comparées</h3><table class="compact"><thead><tr><th>Variante</th><th class="num">Taux nominal</th><th class="num">Assurance / an</th><th class="num">Mensualité immo max.</th><th class="num">Effort / base proposée</th></tr></thead><tbody>${selected
    .map((item) => {
