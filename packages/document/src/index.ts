...
function page(dossier: Dossier, number: number, content: string, className = ""): string {
...
function paragraphs(text: string): string {
  if (/<\/?(?:p|div|strong|b|em|i|ul|ol|li|font|span|br)\b/i.test(text)) return safeRichText(text);
    ...
    .map((line) => {
      ...
      if (trimmed.startsWith("|") && trimmed.endsWith("|")) {
        const cells = trimmed
          ...
          .map((cell) => cell.trim())
          .filter(Boolean);
        return cells.length >= 2 ? `${cells[0]} : ${cells.slice(1).join(" · ")}` : (cells[0] ?? "");
      }
      return line
        .replace(/^\s{0,3}#{1,6}\s+/, "")
        .replace(/^\s*[-*+]\s+/, "• ")
...
function conciseParagraphs(text: string, maximumCharacters: number): string {
  ...
  return paragraphs(`${prefix.slice(0, boundary > maximumCharacters * 0.55 ? boundary + 1 : maximumCharacters).trim()}…`);
}

function editorial(dossier: Dossier, section: SectionKey, position: "introduction" | "callout" | "conclusion", className = ""): string {
  const value = dossier.editorial.sectionSlots[section]?.[position]?.trim();
  if (!value) return "";
  if (position === "callout") return `<div class="callout ${className}">${conciseParagraphs(value, 520)}</div>`;
...
function activityIndicators(activity: ProfessionalActivity): string[] {
  ...
  if (model.kind === "salary")
    return [
      ...(model.contractualGrossAnnualCents != null ? [`Salaire brut annuel : ${formatEuro(model.contractualGrossAnnualCents)}`] : []),
      ...(model.workTimeBasisPoints != null ? [`Quotité : ${formatRate(model.workTimeBasisPoints)}`] : []),
    ];
  if (model.kind === "day-rate")
    return [
      `TJM : ${formatEuro(model.dailyRateCents)} HT`,
      ...(model.billableDaysPerYear != null ? [`${model.billableDaysPerYear} jours facturables / an`] : []),
      ...(model.collectionDelayDays != null ? [`Délai d'encaissement : ${model.collectionDelayDays} jours`] : []),
    ];
  if (model.kind === "consultation")
    return [
      ...(model.consultationFeeCents != null ? [`Tarif par consultation : ${formatEuro(model.consultationFeeCents)}`] : []),
      ...(model.consultationsPerWeek != null ? [`${model.consultationsPerWeek} consultations / semaine`] : []),
      ...(model.workingWeeksPerYear != null ? [`${model.workingWeeksPerYear} semaines / an`] : []),
      ...(model.collectionDelayDays != null ? [`Délai d'encaissement : ${model.collectionDelayDays} jours`] : []),
    ];
  if (model.kind === "turnover")
    return [
      ...(model.annualTurnoverCents != null ? [`CA annuel de référence : ${formatEuro(model.annualTurnoverCents)}`] : []),
      ...(model.collectionDelayDays != null ? [`Délai d'encaissement : ${model.collectionDelayDays} jours`] : []),
    ];
  if (model.kind === "hourly")
    return [
      `Taux horaire : ${formatEuro(model.hourlyRateCents)}`,
      ...(model.hoursPerWeek != null ? [`${model.hoursPerWeek} heures / semaine`] : []),
      ...(model.collectionDelayDays != null ? [`Délai d'encaissement : ${model.collectionDelayDays} jours`] : []),
    ];
...
function documentStatusClass(status: Dossier["supportingDocuments"][number]["status"]): string {
  if (status === "available" || status === "verified") return "document-status--positive";
  if (status === "missing" || status === "requested") return "document-status--attention";
  return "document-status--neutral";
}

function supportingDocumentsTable(dossier: Dossier, people: readonly Dossier["household"]["people"][number][]): string {
  const groups = new Map<Dossier["supportingDocuments"][number]["category"], Dossier["supportingDocuments"]>();
  for (const document of dossier.supportingDocuments) groups.set(document.category, [...(groups.get(document.category) ?? []), document]);
  const ordered = [...groups.entries()].sort(([a], [b]) =>
    a === "other" ? 1 : b === "other" ? -1 : documentCategoryLabels[a].localeCompare(documentCategoryLabels[b], "fr"),
  );
...
function assetsByOwnerTable(dossier: Dossier, people: readonly Dossier["household"]["people"][number][]): string {
  ...
  const orderedKeys = [...people.map((person) => person.id), "household"].filter((key) => groups.has(key));
  const rows = orderedKeys
    .flatMap((key) => {
      const assets = groups.get(key) ?? [];
      const owner = key === "household" ? "Foyer" : (people.find((person) => person.id === key)?.displayName ?? key);
      ...
      const mobilisable = assets
        ...
        .reduce((sum, asset) => sum + (asset.contributionAmountCents ?? asset.amountCents), 0);
      assetRows.push(
    ...
    .join("");
  const total = dossier.assets.reduce((sum, asset) => sum + asset.amountCents, 0);
...
export function renderBankDocument(dossier: Dossier, derived: DerivedDossier): string {
  const sections = dossier.presentation.sections;
  const people = dossier.household.people.filter((person) => person.role !== "dependent");
  const central = derived.financingScenarios.find((scenario) => scenario.id === derived.highlightedScenarioId);
  ...
  if (!central || !centralBudget || !stressBudget)
    throw new Error("Le document exige un scénario principal, un budget central et un budget stress explicites.");
  const centralFinancing = derived.financingScenarios.find(
    (scenario) => scenario.id === (centralBudget.assumptions.financingScenarioId ?? derived.highlightedScenarioId),
  );
  const stressFinancing = derived.financingScenarios.find(
    (scenario) => scenario.id === (stressBudget.assumptions.financingScenarioId ?? derived.highlightedScenarioId),
  );
  if (!centralFinancing || !stressFinancing) throw new Error("Le financement associé à un budget est introuvable.");
  let pageNumber = 0;
  const pages: string[] = [];
  ...
  if (sections.presentationLetter)
    pages.push(
      page(
        dossier,
        ++pageNumber,
  ...
  if (sections.household) {
    const incomeByPerson = new Map(
      people.map((person) => [
        ...
        dossier.incomeStreams
          .filter((income) => income.personId === person.id && income.includedInBorrowingCapacity)
    ...
    pages.push(
      ...
      <table><thead><tr><th>Emprunteur</th><th class="num">Âge</th><th>Profession</th><th>Statut</th><th class="num">Revenu net avant IR retenu</th></tr></thead><tbody>${people
        .map((person) => {
          const activities = dossier.professionalActivities.filter((activity) => activity.personId === person.id);
          return `<tr><td><strong>${escapeHtml(person.displayName)}</strong><br><span class="small">${escapeHtml(personRoleLabels[person.role])}</span></td><td class="num">${ageAt(person.birthDate, dossier.metadata.observationDate)}</td><td>${activities.map((activity) => escapeHtml(activity.occupation)).join("<br>") || "—"}</td><td>${activities.map((activity) => escapeHtml(professionalStatusLabels[activity.status])).join("<br>") || "—"}</td><td class="num">${formatEuro(incomeByPerson.get(person.id) ?? 0)}</td></tr>`;
        })
        .join("")}</tbody></table>
  ...
  if (sections.riskManagement)
    pages.push(
      page(
        ...
        ++pageNumber,
        `
    <h2>Éléments de stabilité et de maîtrise du risque</h2>${editorial(dossier, "riskManagement", "introduction")}<div class="risk-grid">${dossier.editorial.professionalStabilityItems.map((item) => `<div class="risk-card"><h3>${escapeHtml(item.title)}</h3>${conciseParagraphs(item.body, 520)}</div>`).join("")}</div>
  ...
  if (sections.financing)
    ...
    <table class="compact financing-table"><thead><tr><th>Scénario</th><th class="num">Prix</th><th class="num">Frais notaire</th><th class="num">Apport</th><th class="num">Prêt</th><th class="num">Taux</th><th class="num">Assurance</th><th class="num">Mensualité</th><th class="num">Effort central</th><th class="num">Effort prudent</th></tr></thead><tbody>${derived.financingScenarios
      .filter((scenario) => dossier.financingScenarios.find((item) => item.id === scenario.id)?.displayInMainTable !== false)
      .map((scenario) => {
        const source = dossier.financingScenarios.find((item) => item.id === scenario.id);
        const price = source?.priceOverrideCents ?? dossier.project.targetPriceCents;
        const fees = Math.round((price * dossier.project.acquisitionFeeBasisPoints) / 10_000);
  ...
  if (sections.sankey) pages.push(page(dossier, ++pageNumber, `${renderBudgetSankey(dossier, derived)}`, "landscape sankey-page"));

  if (sections.postPurchaseBudget) {
    const stressBySource = new Map(stressBudget.items.map((item) => [item.sourceItemId, item]));
    const centralDebt = centralFinancing.monthlyPaymentIncludingInsuranceCents + centralFinancing.existingDebtAtPurchaseCents;
    const stressDebt = stressFinancing.monthlyPaymentIncludingInsuranceCents + stressFinancing.existingDebtAtPurchaseCents;
    pages.push(
      page(
        dossier,
        ++pageNumber,
      ...
      ${centralBudget.items
        .map((item) => {
          ...
          if (!stressItem) throw new Error(`Poste stress manquant : ${item.id}`);
  ...
  if (sections.supportingDocuments)
    pages.push(
      page(
        dossier,
        ++pageNumber,
  ...
  if (sections.independentIncomeAnnex) {
    ...
    const personSections = independentPeople
      .map((person, personIndex) => {
        const incomes = dossier.incomeStreams.filter((income) => income.personId === person.id && ["self-employed", "liberal"].includes(income.kind));
        const incomeIds = new Set(incomes.map((income) => income.id));
        const histories = dossier.revenueHistory.filter((history) => incomeIds.has(history.incomeStreamId) && history.observed);
        const activities = dossier.professionalActivities.filter(
          (activity) =>
            activity.personId === person.id &&
            ["day-rate", "consultation", "turnover", "hourly", "commission"].includes(activity.compensationModel.kind),
        );
        const latest = histories.at(-1);
        const retained = incomes.reduce((sum, income) => sum + income.monthlyBankCents, 0);
        const modelMetric = activities.flatMap((activity) => {
          ...
          if (model.kind === "consultation" && model.consultationFeeCents != null)
            return [
              {
